...
def get_configs(params)
  ...
  if params[:cluster_name] != $cluster_name
    #printf("rrr 06000%s", cluster_name)
    return JSON.generate({'status' => 'wrong_cluster_name 1' + $cluster_name + ' ' +  params[:cluster_name]})
  end
  out = {
    'status' => 'ok',
...
def set_configs(params)
  ...
  if has_cluster and $cluster_name != configs_json['cluster_name']
    return JSON.generate({'status' => 'wrong_cluster_name 2' + configs_json['cluster_name'] + ' ' + $cluster_name})
  end
